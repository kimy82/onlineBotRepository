...
public class ComandaDaoImpl extends HibernateDaoSupport implements ComandaDao{
	...
	public List<Comandes> getAll(){
		...
		Query query = session.createQuery("from Comandes cmd where ((cmd.revisio IS NOT NULL and cmd.targeta=false and cmd.pagada=true) or (cmd.targeta=true and cmd.pagada=true)) order by cmd.id desc").setCacheable(true);
